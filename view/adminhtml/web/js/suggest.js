...
(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        define([
            'jquery',
            'Magecrafts_AdminSearch/js/hotkeys',
            'mage/template',
            'mage/mage',
            'jquery/ui',
            'mage/backend/menu',
...
}(this, function ($,hotkeys, mageTemplate) {
    'use strict';
    ...
    $.widget('mage.suggest', {
        ...
        prepareTerm:function(term){
            ...
            if(tokens.length ==2 && tokens[0]=='product'){
                this._term = tokens[1];
                return {'entity':'products','query':tokens[1]};
            }
            if(tokens.length ==2 && tokens[0]=='order'){
                this._term = tokens[1];
                return {'entity':'sales','query':tokens[1]};
            }
            if(tokens.length ==2 && tokens[0]=='customer'){
                this._term = tokens[1];
                return {'entity':'customers','query':tokens[1]};
            }
            return {'query':term};
        },
        ...
        _source: function (term, response) {
            ...
            if ($.isArray(o.source)) {
                ...
                ajaxData = this.prepareTerm(term);
